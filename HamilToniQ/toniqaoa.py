...
class Toniq:
    ...
    def QAOA_Graph_Creator(self, label="Standard"):
        graph = nx.Graph()
        if label == "Standard":
    ...
    def get_function_values_sv(self, gamma_values, beta_values, graph, mixer, backendStr, shots):
        ...
        for i, gamma in enumerate(gamma_values):
            for j, beta in enumerate(beta_values):
                theta = [beta, gamma]
                expectation = self.get_expectation_sv(theta, self.QAOA_Graph_Creator(graph), mixer, backendStr, shots)
        ...
        for i in range(len(self.mixers)):
            ...
            for j in range(len(self.problems)):
                values = self.get_function_values(self.gamma_values, self.beta_values, self.problems[j], self.mixers[i], backendStr, self.shots)
        ...
        plt.tight_layout()
        plt.savefig("detailed_analysis.png", dpi=300)  # Save the figure in high resolution
        plt.show()
    ...
    def noisy_simulator_analysis(self):
        ...
        plt.tight_layout()
        plt.savefig("noisy_simulator_analysis.png", dpi=300)  # Save the figure in high resolution
        plt.show()
    ...
    def statevector_analysis(self):
        ...
        for i in range(len(self.mixers)):
            axs[i, 0].set_ylabel('Beta')
            for j in range(len(self.problems)):
                values = self.get_function_values_sv(gamma_values, beta_values, self.problems[j], self.mixers[i], self.backendStr, self.shots)
        ...
        plt.tight_layout()
        plt.savefig("statevector_analysis.png", dpi=300)  # Save the figure in high resolution
        plt.show()
